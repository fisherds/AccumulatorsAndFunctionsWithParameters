...
def better_draw_circles(radius_delta):
    """
    -- Constructs a window whose width and height are both 400.
    -- Constructs and draws 21 rg.Circle objects such that:
         -- Each is centered at (200, 200)
         -- They have radii:  0  1*radius_delta  2*radius_delta etc
         -- Pauses 0.05 seconds after rendering each.
    -- Waits for the user to press the mouse, then closes the window.
    """
    window = rg.RoseWindow(400, 400)
    ...
    center = rg.Point(200, 200)
    for k in range(21):
        circle = rg.Circle(center, radius_delta * k)
        circle.attach_to(window)
        window.render(0.05)  # Pauses for 0.05 seconds after rendering.
...
def test_even_better_draw_circles():
    ...
    print('--------------------------------------------------')
    even_better_draw_circles(8, 12, 'red')
    even_better_draw_circles(2, 100, 'blue')
    even_better_draw_circles(20, 2, 'green')


def even_better_draw_circles(radius_delta, num_circles, color):
    """
    -- Constructs a window whose width and height are both 400.
    -- Constructs and draws num_circles rg.Circle objects such that:
         -- Each is centered at (200, 200)
         -- They have radii:  0  1*radius_delta  2*radius_delta etc
         -- Pauses 0.05 seconds after rendering each.
    -- Waits for the user to press the mouse, then closes the window.
